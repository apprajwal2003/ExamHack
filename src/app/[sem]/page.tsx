...
export default async function SubjectSelector({
  ...
  const semPath = path.join(process.cwd(), "public", "notes", sem);

  let subjects: string[] = [];
  try {
    subjects = fs
      .readdirSync(semPath)
      .filter((item) => fs.statSync(path.join(semPath, item)).isDirectory())
      ...
      <div className="max-w-6xl mx-auto">
        ...
        {subjects.length > 0 ? (
          ...
          <div className="text-center text-gray-500">
            No subjects found. Add folders to <code>public/notes/{sem}</code>.
          </div>
        )}
